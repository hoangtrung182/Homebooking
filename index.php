...
=======
				...
				foreach ($shows as $show) {
					if (isset($_GET['id'])) {
						if ($show['trang_thai'] == 1 && $show['ngay_den'] > $date || $show['trang_thai'] == 0) {
							$list_booking = delete_booking($_GET['id']);
						} else {
							echo "";
						}
					}
		...
		case 'listBookings':
			...
			include './Client/bookings/listBookings.php';
			break;
		case 'confirmBookings':
			...
			if (isset($_GET['xacnhan'])) {
				if ($show['trang_thai'] == 1) {
					...
					$trang_thai = 1;
					$ma_dp = $_GET['xacnhan'];
					$list = update_booking($trang_thai, $ma_dp);
					header("Location:./Client/bookings/listBookings.php");
					exit();
				}
			}
			...
			include './Client/bookings/detailBookings.php';
			break;
		default:
		# code...
	}
